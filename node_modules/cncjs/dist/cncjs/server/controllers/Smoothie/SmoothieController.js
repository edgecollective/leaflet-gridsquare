...
var _SmoothieRunner = _interopRequireDefault(require("./SmoothieRunner"));
...
// % commands
var WAIT = '%wait';
var log = (0, _logger["default"])('controller:Smoothie');
var noop = _noop2["default"];

var SmoothieController = /*#__PURE__*/function () {
  // CNCEngine
  // Sockets
  // Connection
  // Smoothie
  ...
  // Shared context
  // Workflow
  function SmoothieController(engine, options) {
    var _this = this;

    _classCallCheck(this, SmoothieController);

    _defineProperty(this, "type", _constants2.SMOOTHIE);
    ...
    this.runner = new _SmoothieRunner["default"]();
    ...
    this.runner.on('action', function (res) {
      log.debug("action command: action:".concat(res.message));
      ...
      log.error("Unknown action command: action:".concat(res.message));
    });
    ...
    this.runner.on('version', function (res) {
      _this.emit('serialport:read', res.raw);
    });
    ...
    this.queryTimer = setInterval(function () {
      ...
      var zeroOffset = (0, _isEqual2["default"])(_this.runner.getWorkPosition(_this.state), _this.runner.getWorkPosition(_this.runner.state)); // Smoothie settings

      if (_this.settings !== _this.runner.settings) {
        _this.settings = _this.runner.settings;

        _this.emit('controller:settings', _constants2.SMOOTHIE, _this.settings);

        _this.emit('Smoothie:settings', _this.settings); // Backward compatibility
      ...
      if (_this.state !== _this.runner.state) {
        ...
        _this.emit('controller:state', _constants2.SMOOTHIE, _this.state);

        _this.emit('Smoothie:state', _this.state); // Backward compatibility
  ...
  _createClass(SmoothieController, [{
    key: "populateContext",
            ...
            while (1) {
              switch (_context2.prev = _context2.next) {
                ...
                case 14:
                  ...
                  _this2.ready = true; // Initialize controller

                  _this2.initController();
    ...
    value: function addConnection(socket) {
      ...
      if (!(0, _isEmpty2["default"])(this.settings)) {
        // controller settings
        socket.emit('controller:settings', _constants2.SMOOTHIE, this.settings);
        socket.emit('Smoothie:settings', this.settings); // Backward compatibility
      }

      if (!(0, _isEmpty2["default"])(this.state)) {
        // controller state
        socket.emit('controller:state', _constants2.SMOOTHIE, this.state);
    ...
    value: function command(cmd) {
      ...
      var handler = {
        ...
        'gcode:stop': function gcodeStop() {
          ...
          var activeState = (0, _get2["default"])(_this4.state, 'status.activeState', '');

          if (activeState === _constants2.SMOOTHIE_ACTIVE_STATE_HOLD) {
            _this4.writeln('resume'); // resume
        ...
        'sleep': function sleep() {
          _this4.event.trigger('sleep'); // Not supported
        ...
        'feedOverride': function feedOverride() {
          var value = args[0];
          var feedOverride = _this4.runner.state.status.ovF;
          ...
          _this4.runner.state = _objectSpread(_objectSpread({}, _this4.runner.state), {}, {
            status: _objectSpread(_objectSpread({}, _this4.runner.state.status), {}, {
              ovF: feedOverride
            })
          });
        },
        ...
        'spindleOverride': function spindleOverride() {
          var value = args[0];
          var spindleOverride = _this4.runner.state.status.ovS;
        ...
        // Rapid Overrides
        'rapidOverride': function rapidOverride() {// Not supported
        },
        'lasertest:on': function lasertestOn() {
          ...
          _this4.writeln('M3'); // Firing laser at <power>% power and entering manual mode


          _this4.writeln('fire ' + (0, _ensurePositiveNumber["default"])(power));
          ...
          if (duration > 0) {
            ...
            _this4.writeln('G4P' + (0, _ensurePositiveNumber["default"])(duration / 1000)); // Turning laser off and returning to auto mode


            _this4.writeln('fire off');
        ...
        'macro:run': function macroRun() {
          var id = args[0],
              _args$6 = args[1],
              context = _args$6 === void 0 ? {} : _args$6,
              _args$7 = args[2],
              callback = _args$7 === void 0 ? noop : _args$7;
        ...
        'macro:load': function macroLoad() {
          var id = args[0],
              _args$8 = args[1],
              context = _args$8 === void 0 ? {} : _args$8,
              _args$9 = args[2],
              callback = _args$9 === void 0 ? noop : _args$9;
    ...
    value: function writeln(data, context) {
      if ((0, _includes2["default"])(_constants2.SMOOTHIE_REALTIME_COMMANDS, data)) {
  ...
  return SmoothieController;
}();

var _default = SmoothieController;
